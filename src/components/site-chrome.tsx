  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-primary-foreground/10 bg-primary/90 backdrop-blur-md">
      <div className="section-shell grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 py-4 lg:flex lg:justify-between">
        <BrandMark compact />
        <nav className="hidden shrink-0 items-center gap-4 lg:flex" aria-label="Primary navigation">
          <Link to="/" hash="about" className="whitespace-nowrap text-[11px] font-medium uppercase tracking-[0.13em] text-primary-foreground/70 transition-colors hover:text-gold">About</Link>
          <div className="relative" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
            <Button variant="ghost" size="sm" className="h-auto gap-1 whitespace-nowrap px-0 text-[11px] font-medium uppercase tracking-[0.13em] text-primary-foreground/70 hover:bg-transparent hover:text-gold" onClick={() => setServicesOpen((value) => !value)} aria-expanded={servicesOpen}>
           ...
           {anchorLinks.slice(1).map(([label, hash]) => <Link key={hash} to="/" hash={hash} className="whitespace-nowrap text-[11px] font-medium uppercase tracking-[0.13em] text-primary-foreground/70 transition-colors hover:text-gold">{label}</Link>)}
        </nav>
        <div className="flex items-center justify-end gap-2">
          <Button asChild variant="gold" size="sm" className="hidden sm:inline-flex"><Link to="/" hash="contact">Talk to an Advisor <ArrowRight /></Link></Button>
           <Button variant="navIcon" size="icon" className="lg:hidden" onClick={() => setMenuOpen((value) => !value)} aria-label={menuOpen ? "Close menu" : "Open menu"}>{menuOpen ? <X /> : <Menu />}</Button>
        </div>
      </div>
      {menuOpen ? <nav className="border-t border-primary-foreground/10 bg-primary px-4 py-4 lg:hidden" aria-label="Mobile navigation">
